...
//8. Navigate URL -> Login -> CreateCustomer -> CreateProject -> CreateTask ->ModifyTask -> DeleteTask ->  DeleteProject -> DeleteCustomer -> Logout
...
test("scenario 8", async ({ page }) => {
    ...
    //Update Task
    await page.locator("//tbody/tr[3]/td[8]/button[1]").click()
    await page.getByPlaceholder("Enter Task Title").fill("Create Policy")
    await page.getByPlaceholder("Enter Task Description").fill("I am the PH and my daughter is LA who is 8 year old")
    // Cx name dropdown
    await page.selectOption("//select[@class='form-select']", "digit")
    await page.selectOption("(//select[@class='form-select'])[2]", "Digit Icon")
    await page.locator("//button[@type='submit']").click()
    //Veriy task updation
    const TaskUpdated = await page.locator("//td[normalize-space()='Create Policy']").textContent()
    await expect(TaskUpdated).toContain("Create Policy")
    ...
    await page.locator("//tbody/tr[1]/td[7]/button[2]").click()
    await page.waitForTimeout(2000)
